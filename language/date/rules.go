...
var RuleTranslations = map[string]RuleTranslation{
	...
	"ru": {//TODO Check this
		...
		RuleToday:         `сегодня|вечером`,
		RuleTomorrow:      `(после )?завтра`,
		RuleAfterTomorrow: "после",
		RuleDayOfWeek:     `(следующий )?(понедельник|вторник|среда|четверг|пятница|суббота|воскресенье)`,
		RuleNextDayOfWeek: "следующий",
		RuleNaturalDate:   `январь|февраль|март|апрель|май|июнь|июль|август|сентябрь|октябрь|ноябрь|декабрь`,
	},
}
